...
function M_AdmPegawaiPembentukanDataAwalTenagaAdministrasiDetail()
{
	this.dml_type	= 0;
	...
	this.store_agama = new Ext.data.ArrayStore({
			fields		: ['id','name']
		,	url			: m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_d +'data_ref_agama.jsp'
		,	idIndex		: 0
		,	autoLoad	: false
	});
	...
	this.store_gol_darah = new Ext.data.ArrayStore({
			fields		: ['id','name']
		,	url			: m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_d +'data_ref_gol_darah.jsp'
		,	idIndex		: 0
		,	autoLoad	: false
	});
	...
	{
		m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi.panel.layout.setActiveItem(0);
	}
		...
		Ext.Ajax.request({
			...
			,	success :
					function (response)
					{
						...
						m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_list.store.reload();
						m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi.panel.layout.setActiveItem(0);
					}
			,	scope	: this
		});
...
function M_AdmPegawaiPembentukanDataAwalTenagaAdministrasiList()
{
	this.dml_type	= 0;
	...
	this.store = new Ext.data.ArrayStore({
			fields		: this.record
		,	url			: m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_d +'data_list.jsp'
		,	autoLoad	: false
	});
	...
	this.do_add = function()
	{
		m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_detail.do_add();
		m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_detail.do_refresh();
		m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi.panel.layout.setActiveItem(1);
	}
	...
	this.do_refresh = function()
	{
		if (m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_ha_level < 1) {
		...
		if (m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_ha_level < 3) {
			this.btn_edit.setDisabled(true);
		} else {
			this.btn_edit.setDisabled(false);
		}

		if (m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_ha_level >= 2) {
...
function M_AdmPegawaiPembentukanDataAwalTenagaAdministrasi(title)
{
	m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_list	= new M_AdmPegawaiPembentukanDataAwalTenagaAdministrasiList();
	m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_detail	= new M_AdmPegawaiPembentukanDataAwalTenagaAdministrasiDetail();
	
	this.panel = new Ext.Panel({
			id				: 'adm_pegawai_pembentukan_data_awal_tenaga_administrasi_panel'
		,	title			: title
		,	layout			: 'card'
		,	activeItem		: 0
		,	autoWidth		: true
		,	autoScroll		: true
		,	items			: [
				m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_list.panel
			,	m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_detail.panel
			]
	});
	
	this.do_refresh = function(ha_level)
	{
		m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_ha_level = ha_level;
		
		m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi_list.do_refresh();
	}
}

m_adm_pegawai_pembentukan_data_awal_tenaga_administrasi = new M_AdmPegawaiPembentukanDataAwalTenagaAdministrasi('Pembentukan Data Awal Tenaga Administrasi');

//@ sourceURL=adm_pegawai_pembentukan_data_awal_tenaga_administrasi.layout.js
